...
from hivemind.training.trainer_agent import TrainerAgentPytorch
    ...
    with path.open("wb") as output:
        ...
        for game_id in game_ids:
            if version >= 4:
                ...
                from hivemind.data.convert_selfplay_data import SAMPLE_METADATA_V3
...
def test_select_replay_files_uses_all_when_replay_files_is_none_or_all(tmp_path):
    from hivemind.data.convert_selfplay_data import select_replay_files
...
def test_soft_cross_entropy_honors_sample_weights():
    from hivemind.training.trainer_agent import SoftCrossEntropyLoss, SampleWeightedLoss
...
def test_joint_policy_loss_learns_pair_correlation_and_pass_id():
    from hivemind.training.trainer_agent import joint_policy_cross_entropy
...
def test_q_value_blending_in_shard_reading(tmp_path):
    from hivemind.data.convert_selfplay_data import read_binary_shard
