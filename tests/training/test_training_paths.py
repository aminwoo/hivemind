...
import hivemind.training.trainer_agent as trainer_agent_module
import hivemind.training.train_loop as train_loop_module
from hivemind.training.train_loop import (
...
from hivemind.config.train_config import TrainConfig, rl_train_config
from hivemind.training.trainer_agent import (
    TrainerAgentPytorch,
...
def test_default_validation_shard_exists(tmp_path, monkeypatch):
    monkeypatch.setattr(train_loop_module, "project_root", tmp_path)
    preferred = tmp_path / "data/planes/val/evaluation_shard.parquet"
    preferred.parent.mkdir(parents=True)
    preferred.touch()
    assert Path(_resolve_validation_shard()) == preferred
...
def test_train_evaluation_falls_back_to_first_training_shard(tmp_path, monkeypatch):
    monkeypatch.setattr(train_loop_module, "project_root", tmp_path)
    monkeypatch.setitem(main_config, "planes_train_dir", str(tmp_path))
    for filename in ("b.parquet", "a.parquet"):
        (tmp_path / filename).touch()
    assert Path(_resolve_train_evaluation_shard()) == tmp_path / "a.parquet"
