...
from hivemind.cli.train_from_games_parquet import _run_supervised_training
...
def test_supervised_launcher_forwards_staged_dataset_and_model_options(
    ...
    monkeypatch.setattr(
        'hivemind.cli.train_from_games_parquet.subprocess.run',
        fake_run,
    )
    ...
    assert command[1:5] == ['-m', 'hivemind.training.train_loop', '--mode', 'sl']
