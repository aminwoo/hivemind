...
void Bugboard::set(std::string fen) {
...
Bugboard::Bugboard(const Bugboard& board) {
    pos[0] = std::unique_ptr<Stockfish::Position>(new Stockfish::Position);
    pos[1] = std::unique_ptr<Stockfish::Position>(new Stockfish::Position);
...
void Bugboard::undo_move(int board_num) {
...
std::vector<std::pair<int, Stockfish::Move>> Bugboard::legal_moves(Stockfish::Color side) {
