...
private:
    ...
    inline size_t shardIndex(uint64_t hash) const {
        return static_cast<size_t>((hash ^ (hash >> 32)) & (kNumShards - 1));
    }
...
public:
    ...
    std::shared_ptr<Node> lookup(uint64_t hash) const {
        const size_t sIdx = shardIndex(hash);
        const auto& shard = shards_[sIdx];
    ...
    std::shared_ptr<Node> insertOrGet(uint64_t hash, std::shared_ptr<Node> node) {
        ...
        const size_t shardMaxCapacity = maxCapacity_ / kNumShards;
        if (shard.table.size() >= shardMaxCapacity) {
    ...
    bool contains(uint64_t hash) const {
        ...
        const auto& shard = shards_[sIdx];
        std::shared_lock lock(shard.mutex);
        return shard.table.find(hash) != shard.table.end();
    }
    ...
    void reserve(size_t capacity) {
        const size_t shardCap = std::min(capacity, maxCapacity_) / kNumShards;
        for (auto& shard : shards_) {
            std::unique_lock lock(shard.mutex);
            shard.table.reserve(shardCap);
        }
    }
    ...
    int getFullness() const {
        ...
        return static_cast<int>((size() * 1000) / maxCapacity_);
    }
};
