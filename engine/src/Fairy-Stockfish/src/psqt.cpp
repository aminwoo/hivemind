...
void init(const Variant* v) {
  ...
  for (PieceType pt = PAWN; pt <= KING; ++pt)
  {
      ...
      if (isSlider)
      {
          ...
          int r1 = rm + (int(v->maxRank) + int(v->maxFile) - 2 * int(v->capturesToHand)) / 2;
      ...
      for (Square s = SQ_A1; s <= SQ_MAX; ++s)
      {
          ...
          psq[ pc][s] = score + (  pt == PAWN  ? PBonus[std::min(r, RANK_8)][std::min(file_of(s), FILE_H)]
                                 ...
                                 : isSlider    ? make_score(5, 5) * (2 * int(f) + std::max(std::min(int(r), int(v->maxRank) - int(r)), int(RANK_1)) - int(v->maxFile) - 1)
                                 : isPawn      ? make_score(5, 5) * (2 * int(f) - int(v->maxFile))
                                               : make_score(10, 10) * (1 + isSlowLeaper) * (int(f) + std::max(std::min(int(r), int(v->maxRank) - int(r)), int(RANK_1)) - int(v->maxFile) / 2));
