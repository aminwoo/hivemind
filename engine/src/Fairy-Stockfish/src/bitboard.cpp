...
namespace {
  ...
  void init_magics(Bitboard table[], Magic magics[], std::map<Direction, int> directions) {
    ...
    Bitboard* occupancy = new Bitboard[1 << (int(FILE_NB) + int(RANK_NB) - 4)];
    Bitboard* reference = new Bitboard[1 << (int(FILE_NB) + int(RANK_NB) - 4)];
    Bitboard edges, b;
    int* epoch = new int[1 << (int(FILE_NB) + int(RANK_NB) - 4)]();
    int cnt = 0, size = 0;
    ...
    for (Square s = SQ_A1; s <= SQ_MAX; ++s)
    {
        ...
        for (int i = 0; i < size; )
        {
            for (m.magic = 0; popcount((m.magic * m.mask) >> (int(SQUARE_NB) - int(FILE_NB))) < int(FILE_NB) - 2; )
            {
#ifdef LARGEBOARDS
#ifdef PRECOMPUTED_MAGICS
