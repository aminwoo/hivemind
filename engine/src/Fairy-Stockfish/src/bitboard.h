...
constexpr Bitboard rank_bb(Rank r) {
  return Rank1BB << (int(FILE_NB) * int(r));
}
...
constexpr Bitboard forward_ranks_bb(Color c, Square s) {
  return c == WHITE
    ? (AllSquares ^ Rank1BB) << (int(FILE_NB) * int(relative_rank(WHITE, s, RANK_MAX)))
    : (AllSquares ^ rank_bb(RANK_MAX)) >> (int(FILE_NB) * int(relative_rank(BLACK, s, RANK_MAX)));
}
