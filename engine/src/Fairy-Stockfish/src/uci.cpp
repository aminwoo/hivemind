...
namespace {
  ...
  int win_rate_model(Value v, int ply) {
    ...
    double x = std::clamp(double(100 * int(v)) / int(PawnValueEg), -2000.0, 2000.0);
  ...
  //
  // This build ships a fixed variant set and drops the variants.ini parser
  // with it, so there is nothing to load. A here-doc argument is still drained
  // from stdin, otherwise its body would be read back as engine commands.

  void load(istringstream& is, bool = false) {
