...
constexpr Square flip_rank(Square s, Rank maxRank = RANK_8) { // Swap A1 <-> A8
  return Square(int(s) + int(NORTH) * (int(maxRank) - 2 * (int(s) / int(NORTH))));
}

constexpr Square flip_file(Square s, File maxFile = FILE_H) { // Swap A1 <-> H1
  return Square(int(s) + int(maxFile) - 2 * (int(s) % int(NORTH)));
}

constexpr Piece operator~(Piece pc) {
  return Piece(int(pc) ^ int(PIECE_TYPE_NB));  // Swap color of piece B_KNIGHT <-> W_KNIGHT
}
...
constexpr Square make_square(File f, Rank r) {
  return Square(int(r) * int(FILE_NB) + int(f));
}
...
constexpr PieceType type_of(Piece pc) {
  return PieceType(int(pc) & (int(PIECE_TYPE_NB) - 1));
}
...
constexpr File file_of(Square s) {
  return File(int(s) % int(FILE_NB));
}

constexpr Rank rank_of(Square s) {
  return Rank(int(s) / int(FILE_NB));
}
...
constexpr Square to_sq(Move m) {
  return Square(int(m) & int(SQUARE_BIT_MASK));
}
