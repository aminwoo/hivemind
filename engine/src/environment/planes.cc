...
// SIMD-optimized fill for 64 floats/halves (one chess board plane)
template <typename T>
inline void fill_plane(T* curIt, float value) {
    ...
    if constexpr (std::is_same_v<T, float>) {
        ...
        __half hval = __float2half_rn(value);
        unsigned short bits;
        std::memcpy(&bits, &hval, sizeof(bits));
        __m256i val = _mm256_set1_epi16(static_cast<short>(bits));
        for (int i = 0; i < 64; i += 16) {
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(curIt + i), val);
        }
        return;
    }
#endif
...
void board_to_planes_impl(Board& board, T* inputPlanes, Stockfish::Color teamSide, bool hasTimeAdvantage) {
    ...
    if constexpr (std::is_same_v<T, float>) {
        ...
        for (size_t i = 0; i < totalValues; i += 16) {
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(inputPlanes + i), zero);
        }
    } else {
