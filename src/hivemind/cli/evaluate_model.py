...
from hivemind.inference.onnx import load_onnx_model, run_onnx
from hivemind.network import DEFAULT_ONNX_PATH
from hivemind.paths import PROJECT_ROOT
from hivemind.training.data_loaders import RLDataset, load_rl_parquet_shard
from hivemind.training.metrics import Accuracy, AccuracySign, CrossEntropy, MSE
...
def evaluate_model_on_data(session, data_dir, batch_size=64, max_samples=None, augment_flip=False):
    ...
    for pf in tqdm(parquet_files, desc="Processing parquet files"):
        ...
        for i in range(0, len(dataset), batch_size):
            ...
            # Run inference
            outputs = run_onnx(session, batch_x.numpy())
...
def main():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default=str(PROJECT_ROOT / "engine/selfplay_games/training_data_parquet"),
