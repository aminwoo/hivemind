...
The data format uses 74 input channels representing two 37-channel Bughouse
boards from one team's perspective.
...
def planes_to_board_representation(planes: np.ndarray) -> tuple[str, str, dict]:
    ...
    Planes format (74 channels):
    Board A (channels 0-36):
      ...
      31: Time advantage (has sitting)
            ...
            32-36: Last move, halfmove clock, and repetition context

        Board B (channels 37-73): same structure
    ...
    def get_board_representation(board_planes: np.ndarray, board_idx: int) -> tuple[str, dict]:
        """Convert 37 channels for one board to board representation."""
        ...
        # Pocket planes encode count / 16 using uniform uint8 quantization.
        pocket_chars = ['P', 'N', 'B', 'R', 'Q']  # Order: Pawn, Knight, Bishop, Rook, Queen
        our_pocket = []
        opp_pocket = []
        
        for i, char in enumerate(pocket_chars):
            our_count = int(board_planes[12 + i, 0, 0] * 16 / 255 + 0.5)
            opp_count = int(board_planes[17 + i, 0, 0] * 16 / 255 + 0.5)
            if our_count > 0:
    ...
    board_a_planes = planes[:NUM_BUGHOUSE_CHANNELS_PER_BOARD]
    board_b_planes = planes[NUM_BUGHOUSE_CHANNELS_PER_BOARD:]
...
def is_early_game(planes: np.ndarray) -> bool:
    ...
    for board_offset in [0, NUM_BUGHOUSE_CHANNELS_PER_BOARD]:
        ...
        for i in range(5):  # 5 piece types in pocket
            total_pocket_pieces += int(
                planes[board_offset + 12 + i, 0, 0] * 16 / 255 + 0.5
            )
            total_pocket_pieces += int(
                planes[board_offset + 17 + i, 0, 0] * 16 / 255 + 0.5
            )
