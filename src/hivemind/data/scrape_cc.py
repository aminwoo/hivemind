...
from hivemind.data.download import batch_download
from hivemind.paths import DATA_DIRECTORY
...
def download_archive_urls() -> None:
    with open(f"{DATA_DIRECTORY}/leaderboard.txt") as f:
        players = [p.strip() for p in f.readlines()]

    os.makedirs(f"{DATA_DIRECTORY}/archive_urls/", exist_ok=True)

    urls = []
    paths = []
    for player in players:
        urls.append(f"https://api.chess.com/pub/player/{player}/games/archives")
        paths.append(f"{DATA_DIRECTORY}/archive_urls/{player}.json")
...
def download_archive() -> None:
    ...
    os.makedirs(f"{DATA_DIRECTORY}/archives", exist_ok=True)

    urls = []
    paths = []

    for player in players:
        with open(f"{DATA_DIRECTORY}/archive_urls/{player}.json") as f:
            player_urls = json.load(f)["archives"]

        os.makedirs(f"{DATA_DIRECTORY}/archives/{player}/", exist_ok=True)
        for url in player_urls:
            m, y, *_ = url.split('/')[::-1]
            if int(y) >= 2016:
                urls.append(url)
                paths.append(f"{DATA_DIRECTORY}/archives/{player}/{y}_{m}.json")
...
def download_games() -> None:
    ...
    parquet_path = f"{DATA_DIRECTORY}/games.parquet"
    ...
    player_archive_base_glob = str(DATA_DIRECTORY / "archives/{0}/*.json")
