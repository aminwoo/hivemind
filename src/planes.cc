...
struct PlaneData {
    ...
    float* curIt;
    Stockfish::Color us; 
    PlaneData(Board& board, float* inputPlanes, Stockfish::Color us): board(board), inputPlanes(inputPlanes), curIt(inputPlanes), us(us) {}
...
void board_to_planes(Board& board, float* inputPlanes, Stockfish::Color us, bool pass=false) {
    ...
    if (pass) {
        planeData.set_plane_to_value(1.0);
    }
}
