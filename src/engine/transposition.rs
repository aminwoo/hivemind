...
impl<D: IHashData + Copy + Clone> TT<D> {
    ...
    // index's bucket.
    pub fn insert(&mut self, zobrist_key: Zobrist64, data: D) {
        if self.megabytes > 0 {
    ...
    pub fn probe(&self, zobrist_key: Zobrist64) -> Option<&D> {
        if self.megabytes > 0 {
            let index = self.calculate_index(zobrist_key);
            let verification = self.calculate_verification(zobrist_key);
...
impl<D: IHashData + Copy + Clone> TT<D> {
    ...
    fn calculate_index(&self, zobrist_key: Zobrist64) -> usize {
        let key = (zobrist_key.0 & HIGH_FOUR_BYTES) >> SHIFT_TO_LOWER;
    ...
    fn calculate_verification(&self, zobrist_key: Zobrist64) -> u32 {
        (zobrist_key.0 & LOW_FOUR_BYTES) as u32
    }
