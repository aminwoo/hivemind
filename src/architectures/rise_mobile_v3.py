...
from architectures.builder_util import get_act, _ValueHead, _PolicyHead, _Stem, get_se, process_value_policy_head
from configs.train_config import TrainConfig
import torch
import onnx

from utils.constants import POLICY_LABELS
...
class _BottlekneckResidualBlock(Module):
    ...
    def __init__(self, channels, channels_operating, kernel=3, act_type='relu', se_type=None):
        ...
        self.body = Sequential(
            ...
            get_act(act_type),
            Conv2d(in_channels=channels_operating, out_channels=channels_operating, kernel_size=(kernel, kernel),
                   padding=(kernel // 2, kernel // 2), bias=False, groups=channels_operating),
            BatchNorm2d(num_features=channels_operating),
            get_act(act_type),
...
class RiseV3(Module):
    ...
    def __init__(self, nb_input_channels, board_height, board_width,
        ...
        self.policy_head = _PolicyHead(board_height, board_width // 2, channels, channels_policy_head, n_labels,
...
if __name__ == "__main__":
    model = get_rise_v33_model_by_train_config((33, 8, 16), TrainConfig).cuda()
    ...
    dummy_input = torch.randn(8, 33, 8, 16, requires_grad=True).cuda()
    input_names = ['input_planes']
    output_names = ['value_output', 'policy_output']
    torch.onnx.export(model.module, dummy_input, model_path, input_names=input_names, output_names=output_names,
                      export_params=True,
                      opset_version=11,
    ...
    board = BughouseBoard()
    board.set_fen(
        'r1bq1b1r/ppp1k1pp/3npp2/4N1B1/3Q4/2N2N2/PPP2KPP/R6R[NQqbnPPP] w | r1b1k2r/ppp2ppp/2p1p3/6B1/B2nn3/2P1P3/P4PPP/R1B1K2R[pPP] b kq')
    #board.set_time([[1089, 781], [155, 1685]])
    ...
    input_planes = torch.tensor(planes[None, ...], dtype=torch.float).cuda()
    with torch.no_grad():
    ...
    for i in torch.argsort(policy, dim=1, descending=True)[0]:
        if i < len(POLICY_LABELS):
            move = '0-' + POLICY_LABELS[i]
        else:
            move = '1-' + POLICY_LABELS[i - len(POLICY_LABELS)]
