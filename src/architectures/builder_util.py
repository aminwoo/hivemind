...
class _PolicyHead(Module):
    def __init__(self, board_height=11, board_width=11, channels=256, policy_channels=2, n_labels=4992, act_type="relu",
        ...
        lc0_to_az_indices = dict(enumerate(move2planes.make_map('index')))
        az_to_lc0_indices = {az: lc0 for lc0, az in lc0_to_az_indices.items()}
        self.policy_map = torch.LongTensor(
            [lc0 for az, lc0 in sorted(az_to_lc0_indices.items()) if az != -1]).unsqueeze(0).cuda()
    ...
    def forward(self, x):
        ...
        if self.select_policy_from_plane:
            a = self.body(x[:, :, :, :8]).reshape(-1, 5184)[:, self.policy_map].view(-1, 2184)
            b = self.body(x[:, :, :, 8:]).reshape(-1, 5184)[:, self.policy_map].view(-1, 2184)
            return torch.stack((a, b), dim=1).reshape(-1, 2 * 2184)
        else:
            x = self.body(x)
