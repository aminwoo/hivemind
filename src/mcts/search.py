...
class UCTNode():
    ...
    def expand(self, child_priors):
        ...
        #priors = [child_priors[0][0], child_priors[1][0]]
...
def UCT_search(board, eval_fn, team_on_turn, iterations=100, C=np.sqrt(2)):
...
if __name__ == '__main__':
    ...
    state = trainer.load_checkpoint('3')
...
#[[1016, 977], [1001, 993]]
    #board.set_fen('r4rk1/ppp2ppp/4p3/3p2N1/3P1P1n/2N2P1b/PPP2PpP/R2QR1K1[BBPnnp] b - - 4 19', 'r2qkb1r/pppb1bp1/4p3/4N3/7n/2N5/PPP2PPP/R1BQK2R[BPqp] w KQkq - 0 17
    ...
    '''policy_logits, _ = eval_fn(board2planes(board, True)[None, ])
    for i in np.argsort(-policy_logits[1])[0]:
        if i == 0:
            continue
        uci_move = POLICY_LABELS[i]
        if board.turn(1) == chess.BLACK:
            uci_move = mirrorMoveUCI(uci_move)

        move = chess.Move.from_uci(uci_move)
        if move in board.boards[1].legal_moves:
            print(move, policy_logits[1][0][i])'''
